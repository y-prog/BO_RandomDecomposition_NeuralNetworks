...
class WeightAndBiasOptimizer:
    ...
    def weights_biases_loss_coeff(self, layers_list: list, epochs: int, X_data: np.ndarray, y_data: np.ndarray, loss_fn):
        ...
        for it in range(epochs):
            ...
            loss_fn_res = loss_fn(output_forward_prop)
            loss_fn_res_list.append(loss_fn_res)
        return weights_and_biases_list, loss_fn_res_list

    @staticmethod
    def find_best_weights_and_biases(weights_and_biases_list: list, loss_coeff_list: list, metric: str = 'min'):
        if metric == 'min':
