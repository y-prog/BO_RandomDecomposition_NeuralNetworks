...
def eggholder(x, y):
    return (-(y - 5) * np.cos(np.sqrt(abs(x / 2 + (y + 1)) * 20)) - x * np.cos(np.sqrt(abs(x - (y + 50)) * 10))) ** 2

    #return (-(y + 47) * np.sin(np.sqrt(abs(x / 20 + (y + .047)))) - x * np.sin(np.sqrt(abs(x - (y + 47))))) ** 2
    #return (-(y + 47) * np.sin(np.sqrt(abs(x / 2 + (y + 4.7)))) - x * np.sin(np.sqrt(abs(x - (y + 47))))) ** 2
    #return (-(y - 5) * np.sin(np.sqrt(abs(x/2 + (y + 1))*20)) - x * np.cos(np.sqrt(abs(x - (y +50))*10)))**2

lims=300
x_min, x_max = -lims, lims
y_min, y_max = -lims, lims

# Define the number of points per dimension
num_points_per_dimension =30
...
# Usage:
layer_sizes = [4,4,1]   # Define your layer sizes
